    ...
    def load_data(self,dbpre="prism1"):
        ...
        for tb in self.common_tab + self.company_tab + self.report_s_conn_tab + self.report_conn_tab:
            ...
            if tb not in self.common_tab:
                ...
                h_copy_task.write("set hive.auto.convert.join=true; insert into %s.%s select a.* from  %s.%s as a left outer join %s.%s_update as b on a.%s = b.%s where b.%s is null;\n\n"
                                  % (self.dbtoday, tb, dbpre, tb, self.dbtoday, tb, idname, idname,idname))
    ...
    def hive_check_num(self):
        for tb in self.common_tab + self.company_tab + self.report_s_conn_tab + self.report_conn_tab:
            ...
            try:
                ...
                print e
        with open("%s/1compare_num.json" % self.hqlpath, 'a') as fw:
